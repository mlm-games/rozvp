...
const TRANSLATION_KEYS: &[&str] = &[
    ...
    "seed-ice-shroom",
    "seed-doom-shroom",
    // Short packet labels (plain values, safe to extract). The
    // parameterized `hud-*-count` / `award-seed-is` messages are pilot-only:
    // this extractor formats with no args, so they stay out of this list.
